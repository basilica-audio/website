...
- **The bundled IRs are models, not captures.** Nave ships [nine cabinet IRs](#the-bundled-library) inside the plugin, dedicated to the public domain under CC0 1.0 Universal. Every one is computed from an analytical cabinet model rather than recorded from a real cabinet, speaker or microphone — that is what keeps them free of any licensing question. No real-world captures are bundled; when you want the sound of a specific cabinet, load your own capture into either slot.
